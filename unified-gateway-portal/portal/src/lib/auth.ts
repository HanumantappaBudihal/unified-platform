...
// whole suite). The portal is an OAuth client registered on the Platform; it
// brokers auth-code + PKCE and consumes the Platform-issued token.
//
// SSO is enabled only when the issuer + client are configured. Without them the
// portal runs OPEN for local development (middleware does not gate), matching
// the documented dev contract.
